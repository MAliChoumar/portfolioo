...
var PROJECTS = [
  ...
  { badge:"privat", badgeT:{de:"Privates Repository",en:"Private repository"},
    h:"ChoumarOS",
    de:{p:"Eine objektbasierte Plattform, die gerade zu einer mehrmandantenfähigen Global Student Platform wird — kostenlos für Studierende, ganz bewusst: als internationaler Student wäre Geld nehmen ein Gewerbe und eine Rückfrage bei der Ausländerbehörde, bevor auch nur ein Abo verkauft ist. Kein Auftrag — ich wollte wissen, ob ich ein System dieser Größe sauber durchhalte.",
        li:["Objekt-Engine: Objekte, Sub-Objekte, Relationen, Versionen und eine Event-Timeline über acht persönliche Bereiche.","Mandantentrennung erzwingt die Datenbank selbst, nicht nur der Anwendungscode — ein echtes Datenleck zwischen zwei Studierenden fand ein eigener Audit und wurde am selben Tag geschlossen.","671 laufende Unit-Tests, dazu End-to-End-Tests mit automatisierten Barrierefreiheits-Checks (axe-core) und Cross-Browser-Läufen in Firefox und WebKit.","Sechs Quality Gates vor jedem Release — Format, Lint, Typecheck, Unit, E2E, Build — bei striktem TypeScript ohne any; dazu verschlüsselte tägliche Backups und 24 dokumentierte Architekturentscheidungen."]},
    en:{p:"An object-based platform becoming a multi-tenant Global Student Platform — free for students, deliberately: as an international student, charging money would mean registering as a trader and a question for the immigration office before a single subscription sells. Not a commission — I wanted to know whether I could hold a system of this size together cleanly.",
        li:["Object engine: objects, sub-objects, relations, versions and an event timeline across eight personal workspaces.","Tenant isolation is enforced by the database itself, not just application code — a real cross-student data leak was caught by an internal audit and closed the same day.","671 passing unit tests, plus end-to-end tests with automated accessibility checks (axe-core) and cross-browser runs in Firefox and WebKit.","Six quality gates before every release — format, lint, typecheck, unit, e2e, build — under strict TypeScript with no any; plus encrypted daily backups and 24 documented architecture decisions."]},
    chips:["Next.js","TypeScript","Drizzle ORM","PostgreSQL","Supabase","Zod","Playwright","Vitest","ESLint"] },
